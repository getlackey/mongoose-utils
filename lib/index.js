...
MongooseUtils.mergeData = function (newObj, method) {
    console.log('Deprecated: MongooseUtils.mergeData. Please use .update or .patch');
...
MongooseUtils.patch = function (newObj) {
    // if it's a mongoose object, convert it to a literal
    if (newObj.toObject) {
    ...
    return function (mongooseObj) {
        ...
        merged = merge(mongooseObj, newObj);

        return merged;
    };
};

MongooseUtils.update = function (newObj) {
    // if it's a mongoose object, convert it to a literal
    if (newObj.toObject) {
        newObj = newObj.toObject();
    }
    ...
    return function (mongooseObj) {
        ...
        if (mongooseObj.toObject) {
            oldObj = mongooseObj.toObject();
        }

        // Delete previous data in db, except the private properties
